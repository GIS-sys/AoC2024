DEBUG = True
INTERMEDIATE_AMOUNT = 25
MAX_LOOKUP_SIZE = 410


class Keypad:
    @classmethod
    def find_specific_sequence(cls, code: str, amount: int = 0) -> tuple[str, int]:
        code = "A" + code
        sequence = ""
        weight = 0
        for i in range(0, len(code) - 1):
            key_from, key_to = code[i], code[i + 1]
            part_sequence, part_weight = cls.how_to_press(key_from, key_to, amount)
            sequence += part_sequence
            weight += part_weight
        return sequence, weight

    @classmethod
    def how_to_press(cls, key_from: str, key_to: str, amount: int = 0) -> tuple[str, int]:
    ...
    @classmethod
    def how_to_press_pos(cls, pos_from_x: int, pos_from_y: int, pos_to_x: int, pos_to_y: int, amount: int = 0) -> tuple[str, int]:
        return "?", 0
...
class DirectionalIntermediateKeypad(DirectionalKeypad):
    ...
    def how_to_press_pos(cls, pos_from_x: int, pos_from_y: int, pos_to_x: int, pos_to_y: int, amount: int = 0) -> tuple[str, int]:
        ...
        if pos_from_y == cls.BAD_POS[1] and pos_to_x == cls.BAD_POS[0]:
            result = ver + hor + "A"
            result = DirectionalIntermediateKeypad.find_specific_sequence(result, amount - 1)
        ...
        if pos_from_x == cls.BAD_POS[0] and pos_to_y == cls.BAD_POS[1]:
            ...
            if len(cls.LOOKUP) < MAX_LOOKUP_SIZE:
                cls.LOOKUP[args] = result
            return result
        hor_first = DirectionalIntermediateKeypad.find_specific_sequence(hor + ver + "A", amount - 1)
        ver_first = DirectionalIntermediateKeypad.find_specific_sequence(ver + hor + "A", amount - 1)
        if hor_first[1] < ver_first[1]:
            result = hor_first
        else:
            result = ver_first
        if len(cls.LOOKUP) < MAX_LOOKUP_SIZE:
...
class NumericKeypad(Keypad):
    ...
    def how_to_press_pos(cls, pos_from_x: int, pos_from_y: int, pos_to_x: int, pos_to_y: int, amount: int = 0) -> tuple[str, int]:
        ...
        if pos_from_y == cls.BAD_POS[1] and pos_to_x == cls.BAD_POS[0]:
            result = ver + hor + "A"
            return DirectionalIntermediateKeypad.find_specific_sequence(result, amount=INTERMEDIATE_AMOUNT)
        if pos_from_x == cls.BAD_POS[0] and pos_to_y == cls.BAD_POS[1]:
            result = hor + ver + "A"
            return DirectionalIntermediateKeypad.find_specific_sequence(result, amount=INTERMEDIATE_AMOUNT)
        hor_first = DirectionalIntermediateKeypad.find_specific_sequence(hor + ver + "A", amount=INTERMEDIATE_AMOUNT)
        ver_first = DirectionalIntermediateKeypad.find_specific_sequence(ver + hor + "A", amount=INTERMEDIATE_AMOUNT)
        if hor_first[1] < ver_first[1]:
            return hor_first
        else:
            return ver_first
...
class Solver:
    ...
    def find_sequence(self, code: str):
        result = NumericKeypad.find_specific_sequence(code)[0]
        return result
